...
var OpusEncoder = require( '@discordjs/opus' ).OpusEncoder;
...
var MumbleConnection = function( socket, options ) {
    ...
    this.channels = {};
    this.users = {};
    this.packetBuffer = Buffer.allocUnsafe( this.FRAME_SIZE * 2 );
    this.voiceBuffer = [];
...
MumbleConnection.prototype.sendEncodedFrame = function(
    ...
    var voiceHeader = Buffer.allocUnsafe( 1 + sequenceVarint.length );
    ...
    for( var i = 0; i < packets.length; i++ ) {
        ...
        if( codec === MumbleConnection.codec.Opus ) {
            ...
            header = Buffer.allocUnsafe( [ packet.length | ( terminator ? 0 : 0x10 ) ] );
        }

        var frame = Buffer.allocUnsafe( header.length + packet.length );
        header.copy( frame, 0 );
...
MumbleConnection.prototype._mix = function( decoded ) {
    ...
    if( decoded.length === 1 ) {
        ...
        mixed = Buffer.allocUnsafe( this.FRAME_SIZE * 2 );
